...
class H5Matrix(dict):
    ...
    def open(self, fname):
        ...
        if isinstance(h5.h5file.getNodeAttr('/','tables'), (list, tuple)):
            h5.matrices = int(h5.h5file.getNodeAttr('/','tables')[0])
        elif isinstance(h5.h5file.getNodeAttr('/','tables'), numpy.ndarray):
            h5.matrices = int(h5.h5file.getNodeAttr('/','tables')[0])
        else:
            h5.matrices = h5.h5file.getNodeAttr('/','tables')
        h5.populate_tables()
    ...
    def create(self, fname, zones=0, matrices=0, tnames=None, template=None):
        ...
        h5.h5file.setNodeAttr('/','tables', numpy.array([h5.matrices], numpy.int32))
    ...
    def populate_tables(self):
        ...
        for t in range(1,self.matrices+1):
            ...
            self[t] = node
            self.tnames.append(self[t].attrs.name)

        # if a table name was missing, forget it
        if len(self.tnames) != self.matrices:
            self.tnames = None
